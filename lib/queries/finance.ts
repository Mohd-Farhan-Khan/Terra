...
export async function fetchActiveCategories() {
  ...
  throwIfError(result.error);
  return (result.data ?? []) as (Option & { kind: "income" | "expense" })[];
}

export async function fetchTransactionFilters() {
  const [accounts, categories] = await Promise.all([fetchActiveAccounts(), fetchActiveCategories()]);
  return { accounts, categories: categories.filter((category) => category.kind === "expense") };
}

export async function fetchTransactions(filters: { fromDate: string; toDate: string; accountId: string; categoryId: string; page: number; pageSize: number }) {
  const client = createClient();
  let ledger = client.from("transaction_list").select("transaction_id,date,account_id,account_name,category_id,category_name,note,type,direction,amount", { count: "exact" }).order("date", { ascending: false }).range(filters.page * filters.pageSize, filters.page * filters.pageSize + filters.pageSize - 1);
  ...
  const [ledgerResult, summaryResult] = await Promise.all([
    ledger,
    client.rpc("transaction_summary", { p_from_date: filters.fromDate || null, p_to_date: filters.toDate || null, p_account_id: filters.accountId || null, p_category_id: filters.categoryId || null }).maybeSingle(),
  ]);
...
export async function fetchDashboard(month: string, nextMonth: string) {
  ...
  const [summary, netWorth, trend, categories, budgets, bills] = await Promise.all([
    client.from("dashboard_monthly_summary").select("month,income_amount,expense_amount,net_amount,savings_rate_percent").eq("month", month).maybeSingle(),
    ...
    client.from("category_monthly_spend_with_share").select("category_id,actual_spend,share_percent").eq("month", month).order("actual_spend", { ascending: false }).limit(6),
    client.from("budget_vs_actual_with_progress").select("category_id,planned_amount,carry_in,available_amount,actual_spend,remaining_amount,progress_percent,utilization_percent").eq("month", month).order("utilization_percent", { ascending: false }),
    client.from("upcoming_recurring_bills").select("recurring_rule_id,name,amount,next_due_date,days_until_due").order("next_due_date").limit(6),
  ]);
  throwIfError(summary.error ?? netWorth.error ?? trend.error ?? categories.error ?? budgets.error ?? bills.error);
  const categoryIds = [...new Set([...(categories.data ?? []).map((row) => row.category_id), ...(budgets.data ?? []).map((row) => row.category_id)])];
  const names = categoryIds.length ? await client.from("categories").select("id,name").in("id", categoryIds) : { data: [], error: null };
  throwIfError(names.error);
  return { summary: summary.data, netWorth: netWorth.data, trend: trend.data ?? [], categories: categories.data ?? [], budgets: budgets.data ?? [], bills: bills.data ?? [], categoryNames: names.data ?? [] };
}
...
export async function fetchAccountOverview() {
  ...
  const [accounts, transactions] = await Promise.all([
    ...
    client.from("transactions").select("account_id,amount,direction,date,created_at").eq("status", "completed").order("date").order("created_at"),
  ]);
...
export async function fetchBudgets(month: string) {
  const client = createClient();
  const budgets = await client.from("budget_vs_actual_with_progress").select("category_id,month,planned_amount,carry_in,available_amount,actual_spend,remaining_amount,rollover_enabled,progress_percent,utilization_percent").eq("month", month).order("category_id");
  throwIfError(budgets.error);
  const categoryIds = (budgets.data ?? []).map((row) => row.category_id);
  const categories = categoryIds.length ? await client.from("categories").select("id,name").in("id", categoryIds) : { data: [], error: null };
...
export async function fetchCounterparties() {
  ...
  const [counterparties, balances] = await Promise.all([
    ...
    client.from("account_balances").select("account_id,account_type,balance").in("account_type", ["receivable", "payable"]),
  ]);
...
export async function fetchCounterpartyStatement(accountId: string) {
  const result = await createClient().from("transactions").select("id,amount,direction,type,date,note,created_at").eq("account_id", accountId).eq("status", "completed").order("date", { ascending: false }).order("created_at", { ascending: false });
  throwIfError(result.error);
  return result.data ?? [];
}
...
export async function fetchRecurring(month: string, nextMonth: string) {
  ...
  const [rules, transactions] = await Promise.all([
    client.from("recurring_rules").select("id,name,account_id,category_id,amount,frequency,next_due_date,is_active").eq("is_active", true).order("next_due_date"),
    client.from("transactions").select("recurring_rule_id,amount,date").eq("status", "completed").not("recurring_rule_id", "is", null).gte("date", month).lt("date", nextMonth),
  ]);
